...
def checkLoot():
  ...
  #lootWin = pyautogui.screenshot(region=(1417, 617, 315, 99))
  time.sleep(1.5)
  #if(lootWin == None):
    #print("not found")
  #else:
  ...
  for col in range (0,2):
    for row in range(0,6): 
      for image in lootFiles:
        obtained = pyautogui.locate("loot_images/%s" % image, lootWin, region=((row*46)+(row*8), (col*46)+(col*8), 46, 46), confidence=0.95  )
        if (obtained != None):
          print(image)
          #if press R loot
          pressR()
...
def checkRod():
  ...
  if(bustedHeld == None):
    ...
    #screenshot Inv to search for rods
